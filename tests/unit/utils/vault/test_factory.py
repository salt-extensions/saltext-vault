...
class TestGetAuthdClient:
    ...
    def build_succeeds(self, client_valid, request):
        with patch("saltext.vault.utils.vault.factory._build_authd_client", autospec=True) as build:
    ...
    def clear_cache(self):
        with patch("saltext.vault.utils.vault.factory.clear_cache", autospec=True) as clear:
    ...
    def test_get_authd_client_exception(
        ...
        assert client.token_valid()
        if session:
            clear_cache.assert_called_once_with({}, ANY, force_local=False, session=session)
        else:
            clear_cache.assert_called_once_with({}, ANY, force_local=False, connection=True)
    ...
    def test_get_authd_client_unwrap_exception(self, events):
        ...
        with patch(
            ...
            with patch(
                "saltext.vault.utils.vault.factory._build_authd_client", autospec=True
            ) as build:
    ...
    def test_get_authd_client_renewable_token_max_ttl_insufficient(self, clear_cache):
        """
...
class TestBuildAuthdClient:
    ...
    @pytest.fixture(autouse=True)
    def cbank(self):
        with patch("saltext.vault.utils.vault.cache._get_cache_bank", autospec=True) as cbank:
    ...
    def conn_config(self):
        with patch(
            "saltext.vault.utils.vault.factory._get_connection_config", autospec=True
        ) as conn_config:
            yield conn_config
    ...
    def fetch_secret_id(self, secret_id_response):
        with patch(
            "saltext.vault.utils.vault.factory._fetch_secret_id", autospec=True
        ) as fetch_secret_id:
    ...
    def fetch_token(self, token_auth):
        with patch("saltext.vault.utils.vault.factory._fetch_token", autospec=True) as fetch_token:
    ...
    def cached(self, token_auth, secret_id_response, request):
        ...
        # Save a reference to the original class since it is
        ...
        with patch("saltext.vault.utils.vault.cache.VaultAuthCache", cache):
            yield cache
    ...
    def test_build_authd_client(self, test_remote_config, conn_config, fetch_secret_id, cached):
        """
...
class TestGetConnectionConfig:
    ...
    @pytest.fixture
    def local(self):
        with patch("saltext.vault.utils.vault.factory._use_local_config", autospec=True) as local:
            yield local

    @pytest.fixture
    def remote(self, test_remote_config, unauthd_client_mock):
        with patch("saltext.vault.utils.vault.factory._query_master") as query:
            query.return_value = (test_remote_config, unauthd_client_mock)
            yield query

    @pytest.mark.usefixtures("salt_runtype")
    @pytest.mark.parametrize(
    ...
    def test_get_connection_config_local(self, force_local, local):
        """
    ...
    def test_get_connection_config_cached(self, cached, remote, test_remote_config):
        ...
        (
            ...
            embedded_token,
            _,
        ) = vfactory._get_connection_config("vault", {}, {})
    ...
    @pytest.mark.parametrize("test_remote_config", ["token"], indirect=True)
    def test_get_connection_config_update_server(self, cached, remote, test_remote_config):
        """
    ...
    @pytest.mark.parametrize("test_remote_config", ["token"], indirect=True)
    def test_get_connection_config_update_auth_method(self, cached, remote, test_remote_config):
        """
        Ensure updating connection config does not work without clearing
        the session if fundamental details have changed
        """
    ...
    @pytest.mark.parametrize("test_remote_config", ["token"], indirect=True)
    def test_get_connection_config_update_cache_backend(self, cached, remote, test_remote_config):
        """
        Ensure updating connection config does not work without clearing
        the session if fundamental details have changed
        """
    ...
    @pytest.mark.parametrize("test_remote_config", ["approle"], indirect=True)
    def test_get_connection_config_update_bind_secret_id(self, cached, remote, test_remote_config):
        """
        Ensure updating connection config does not work without clearing
        the session if fundamental details have changed
        """
...
class TestFetchSecretId:
    ...
    @pytest.mark.parametrize("test_remote_config", ["approle"], indirect=True)
    def test_fetch_secret_id_cached(self, test_remote_config, cached, remote, unauthd_client_mock):
        """
        Ensure cache is respected
        """
        res = vfactory._fetch_secret_id(test_remote_config, {}, cached, unauthd_client_mock)
    ...
    def test_fetch_secret_id_uncached(
        ...
        """
        res = vfactory._fetch_secret_id(test_remote_config, {}, uncached, unauthd_client_mock)
        uncached.store.assert_called_once()
        remote.assert_called_once()
        data, _ = remote()
    ...
    def test_fetch_secret_id_config_location(
        ...
        if conf_location == "doesnotexist":
            with pytest.raises(
                ...
                vfactory._fetch_secret_id(test_remote_config, opts, uncached, unauthd_client_mock)
        else:
            vfactory._fetch_secret_id(test_remote_config, opts, uncached, unauthd_client_mock)
            if called:
                remote.assert_called()
...
class TestFetchToken:
    ...
    def remote(self, token_auth, server_config, unauthd_client_mock):
        with patch("saltext.vault.utils.vault.factory._query_master", autospec=True) as query:
            query.return_value = (
    ...
    @pytest.mark.usefixtures("salt_runtype")
    @pytest.mark.parametrize("test_remote_config", ["token", "wrapped_token"], indirect=True)
    @pytest.mark.parametrize(
        "salt_runtype,force_local",
        [
            ("MASTER", False),
    ...
    @pytest.mark.usefixtures("salt_runtype")
    @pytest.mark.parametrize("test_remote_config", ["token", "token_changed"], indirect=True)
    @pytest.mark.parametrize(
        "salt_runtype,force_local",
        [
            ("MASTER", False),
    ...
    def test_fetch_token_local_cached_changed(
        ...
        # with patch("saltext.vault.utils.vault.VaultClient.token_lookup") as token_lookup:
        unauthd_client_mock.token_lookup.return_value = _mock_json_response(
            token_lookup_self_response, status_code=200
        )
        res = vfactory._fetch_token(
    ...
    @pytest.mark.parametrize("test_remote_config", ["token", "wrapped_token"], indirect=True)
    def test_fetch_token_cached(self, test_remote_config, cached, remote, unauthd_client_mock):
        """
        Ensure that cache is respected
        """
        res = vfactory._fetch_token(test_remote_config, {}, cached, unauthd_client_mock)
        assert res == cached.get()
        cached.store.assert_not_called()
        remote.assert_not_called()
    ...
    @pytest.mark.parametrize("test_remote_config", ["token"], indirect=True)
    def test_fetch_token_uncached(self, test_remote_config, uncached, remote, unauthd_client_mock):
        """
        Test that tokens that were sent with the connection configuration
        are used when no cached token is available
        """
        test_remote_config["auth"].pop("token", None)
        res = vfactory._fetch_token(test_remote_config, {}, uncached, unauthd_client_mock)
        uncached.store.assert_called_once()
        remote.assert_called_once()
...
class TestQueryMaster:
    ...
    def unwrap_client(
        ...
        # first, its spec is sourced from a MagicMock (and fail in Python >=3.11)
    ...
    def test_query_master_uses_correct_module(
        ...
        """
        out, _ = vfactory._query_master("func", opts)
    ...
    def test_query_master_invalidates_cache_when_expected_server_differs(
        ...
        with pytest.raises(vault.VaultConfigExpired):
            vfactory._query_master("func", opts, unwrap_client=unauthd_client_mock)
    ...
    def test_query_master_merges_unwrapped_result(
        ...
        out, _ = vfactory._query_master("func", opts, unwrap_client=unauthd_client_mock)
...
class TestBuildRevocationClient:
    ...
    def token(self, token_auth, request):
        with patch("saltext.vault.utils.vault.cache.VaultAuthCache", autospec=True) as cache:
            if request.param:
    ...
    def test_build_revocation_client_never_calls_master_for_config(self):
        with patch("saltext.vault.utils.vault.factory._query_master") as query:
            vfactory._build_revocation_client({}, {})
...
def test_use_local_config(test_config, expected_config, expected_token):
    ...
    with patch("saltext.vault.utils.vault.factory.parse_config", Mock(return_value=test_config)):
        output, token, _ = vfactory._use_local_config({})
